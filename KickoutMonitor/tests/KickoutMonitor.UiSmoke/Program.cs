...
internal static class Program
{
    ...
    private static int Main()
    {
        try
        {
            ...
            Require(vm.IncludeInTraining, "segmentation overkill selection restores checked");
            vm.HandleHotkey(Key.Enter);
            ...
            Require(chart.RenderedPointCount == 6 && chart.RenderedSegmentCount == 4, $"zeros render and missing day breaks lines ({chart.RenderedPointCount}/{chart.RenderedSegmentCount}; VM {overkill.Kickout.Series.Sum(s=>s.Points.Count(p=>p.HasData))}; status {overkill.Status})");
            Require(Descendants<KickoutMonitor.App.OverkillLineChart>(trendView).Count() == 8 && overkill.Kickout.Cards.Count == 8, "eight separate charts and tables");
            Require(overkill.Kickout.Cards.All(c => c.Rows.Select(r => r.Count ?? -1).SequenceEqual(c.Rows.Select(r => r.Count ?? -1).OrderDescending())), "tables sorted by descending count");
            Require(overkill.Fields[1] == "SEPA", "field list sorted by total count");
            ...
            var smallerChartHeight = trendView.ActualHeight;
            var close = Descendants<Button>(dashboard).Single(b => b.Content as string == "닫기 ×");
            close.Command.Execute(null);
            Layout(dashboard);
            Require(!overkill.DetailsOpen && trendView.ActualHeight > smallerChartHeight, "closing detail restores chart space");
